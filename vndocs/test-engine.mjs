...
/* ── Só chave dupla ───────────────────────────────────────────────────── */
{
  ...
  const textoDe = (buf) =>
    new PizZip(buf).files['word/document.xml'].asText().replace(/<[^>]*>/g, '');

  const dupla = monta('Cliente: {{Nome}} / {{Pedido}}');
  check('chave dupla é reconhecida', contarCampos(dupla).campos.length === 2);
  check('chave dupla preenche',
    textoDe(fillDocument(dupla, { Nome: 'Gustavo', Pedido: 'GG1' })).includes('Gustavo'));

  /* Chave simples NÃO é campo: "{" aparece por acaso em texto comum, e
     tratá-lo como marcador estragaria o documento. */
  const solta = monta('Total: { valor entre chaves } e {{Nome}}');
  const nomes = contarCampos(solta).campos.map((c) => c.nome);
  check('chave simples não vira campo',
    nomes.length === 1 && nomes[0] === 'Nome', nomes.join(', '));
  check('o trecho entre chave simples fica intacto',
    textoDe(fillDocument(solta, { Nome: 'Gustavo' })).includes('{ valor entre chaves }'));
}
