...
import Tour, { tourJaVisto } from '../components/ui/Tour';
...
function StatCard({ Icon, label, value, sub, good }) {
  /* minWidth: 0 é o que impede o cartão de empurrar a grade e sobrepor. */
  return (
    <div className="hoverable" style={{ background: 'var(--n0)', border: '1px solid var(--n3)', borderRadius: 'var(--r-m)', padding: '18px 18px', minWidth: 0, boxShadow: '0 1px 2px var(--n2)' }}>
      <div style={{ width: 38, height: 38, borderRadius: 11, background: 'var(--n2)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--n6)', marginBottom: 16 }}>
        <Icon size={18} strokeWidth={1.8} />
      </div>
      <div style={{ fontSize: 13, color: 'var(--n6)', marginBottom: 6 }}>{label}</div>
      <div style={{ fontFamily: "'Space Grotesk', sans-serif", fontWeight: 700, fontSize: 34, letterSpacing: '-0.04em', lineHeight: 1 }}>{value}</div>
      <div style={{ fontSize: 12.5, color: good ? 'var(--verde)' : 'var(--n6)', marginTop: 8, fontWeight: good ? 500 : 400 }}>{sub}</div>
    </div>
  );
}
...
export default function Dashboard() {
  ...
  /* O tutorial abre sozinho na primeira visita. Apontar para os botões de
     verdade ensina mais que uma tela de boas-vindas que ninguém lê — e ele
     não volta depois de visto ou pulado. */
  const [tourAberto, setTourAberto] = useState(false);
  useEffect(() => {
    if (tourJaVisto()) return undefined;
    /* Espera a barra lateral montar, senão não há o que destacar. */
  ...
  return (
    <AppLayout>
      <div style={{ display: 'flex', flexDirection: 'column', height: '100%', overflow: 'hidden' }}>
        ...
        {/* ── Body ── */}
        <div className="stagger" style={{ flex: 1, overflowY: 'auto', padding: isMobile ? '16px 14px' : '24px 28px', display: 'flex', flexDirection: 'column', gap: 22 }}>
          {/* Hero banner */}
          <div style={{ position: 'relative', flexShrink: 0, borderRadius: 22, overflow: 'hidden', background: 'linear-gradient(120deg,var(--azul-forte) 0%,var(--azul) 55%,#3B82F6 100%)', padding: '32px 36px', display: 'flex', flexWrap: 'wrap', gap: 32, color: 'var(--n0)' }}>
            ...
            <div style={{ flex: 1, minWidth: isMobile ? 0 : 320, position: 'relative', zIndex: 1 }}>
              ...
              <div style={{ display: 'flex', gap: 12 }}>
                <button onClick={() => navigate('/generate')} style={{ display: 'flex', alignItems: 'center', gap: 9, height: 48, padding: '0 24px', borderRadius: 12, background: 'var(--n0)', color: 'var(--azul-forte)', border: 'none', fontSize: 15, fontWeight: 700, cursor: 'pointer', boxShadow: '0 8px 22px rgba(0,0,0,.18)' }}>
                  <Zap size={18} strokeWidth={2.4} /> Começar geração
                </button>
                <button onClick={() => setGuiaAberto(true)} style={{ display: 'flex', alignItems: 'center', gap: 9, height: 48, padding: '0 22px', borderRadius: 12, background: 'rgba(255,255,255,.12)', color: 'var(--n0)', border: '1px solid rgba(255,255,255,.3)', fontSize: 15, fontWeight: 600, cursor: 'pointer' }}
            ...
            <div style={{ width: isMobile ? '100%' : 320, flexShrink: 0, position: 'relative', zIndex: 1, paddingLeft: 32, borderLeft: '1px solid rgba(255,255,255,.18)' }}>
              ...
              <div style={{ width: '100%', height: 8, background: 'rgba(255,255,255,.2)', borderRadius: 4, overflow: 'hidden', marginBottom: 20 }}>
                <div style={{ height: '100%', width: `${usePct}%`, background: 'var(--n0)', borderRadius: 4 }} />
              </div>
          ...
          <div className="stagger" style={{
            ...
            gap: isMobile ? 10 : 18, flexShrink: 0,
          }}>
            {carregandoPainel
              ? <div style={{ gridColumn: '1 / -1', padding: '32px 0', display: 'flex', justifyContent: 'center' }}><Spinner size={24} /></div>
              : STATS.map(s => <StatCard key={s.label} {...s} />)}
          </div>

          {/* Recent activity */}
          <div style={{ background: 'var(--n0)', border: '1px solid var(--n3)', borderRadius: 18, padding: '22px 24px', boxShadow: '0 1px 2px var(--n2)' }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 18 }}>
              <div>
                ...
                <p style={{ fontSize: 12.5, color: 'var(--n6)', marginTop: 2 }}>Últimas gerações do workspace</p>
              </div>
              <button onClick={() => navigate('/history')} style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 13, fontWeight: 500, color: 'var(--azul)', background: 'none', border: 'none', cursor: 'pointer' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column' }}>
              {ACTIVITY.length === 0 && (
                <div style={{ padding: '34px 10px', textAlign: 'center', fontSize: 13, color: 'var(--n6)' }}>
                  Nenhum lote gerado ainda.
                </div>
              )}
              {ACTIVITY.map((a, i) => (
                <div key={a.id} className="row-hover" style={{ display: 'flex', alignItems: 'center', gap: 14, padding: '14px 10px', borderTop: i === 0 ? 'none' : '1px solid var(--n3)' }}>
                  <div style={{ width: 38, height: 38, borderRadius: 10, background: 'var(--n2)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--n6)', flexShrink: 0 }}>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <div style={{ fontSize: 12, color: 'var(--n6)', marginTop: 3 }}>{a.folder}</div>
                  </div>
                  {a.count && (
                    <span style={{ fontSize: 11.5, fontWeight: 600, color: 'var(--azul)', background: 'var(--azul-veu)', borderRadius: 999, padding: '3px 11px', flexShrink: 0 }}>{a.count} arquivos</span>
                  )}
                  <span style={{ fontSize: 12.5, color: 'var(--n6)', minWidth: 78, textAlign: 'right', flexShrink: 0 }}>{a.time}</span>
                  <button onClick={() => navigate('/history')} title="Ver no histórico" style={{ width: 32, height: 32, borderRadius: 8, background: 'none', border: 'none', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--n6)', cursor: 'pointer', flexShrink: 0 }}
                    onMouseEnter={e => { e.currentTarget.style.background = 'var(--n2)'; e.currentTarget.style.color = 'var(--azul)'; }} onMouseLeave={e => { e.currentTarget.style.background = 'none'; e.currentTarget.style.color = 'var(--n6)'; }}>
                    <Download size={16} />
                  </button>
                </div>
              ))}
      ...
      {tourAberto && <Tour aoFechar={() => setTourAberto(false)} />}
        ...
        footer={
          <div style={{ display: 'flex', gap: 10, justifyContent: 'flex-end' }}>
            ...
            <button onClick={() => { setGuiaAberto(false); navigate('/generate'); }}
              style={{ height: 38, padding: '0 18px', borderRadius: 999, background: 'var(--azul)', color: 'var(--n0)', border: 'none', fontFamily: 'inherit', fontSize: 13.5, fontWeight: 600, cursor: 'pointer' }}>
              Começar agora
            </button>
          </div>
        }
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
          {[
            ...
            <div key={n} style={{ display: 'flex', gap: 14 }}>
              <div style={{ width: 28, height: 28, borderRadius: '50%', background: 'var(--azul-veu)', color: 'var(--azul)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 700, fontSize: 13, flexShrink: 0 }}>
                {n}
              </div>
              <div>
                <div style={{ fontWeight: 600, fontSize: 14.5, marginBottom: 4 }}>{titulo}</div>
                <p style={{ fontSize: 13.5, lineHeight: 1.6, color: 'var(--n7)', margin: 0 }}>{texto}</p>
              </div>
            </div>
          ))}
          <div style={{ background: 'rgba(37,99,235,.06)', border: '1px solid rgba(37,99,235,.14)', borderRadius: 10, padding: '12px 14px', fontSize: 13, color: 'var(--n7)', lineHeight: 1.55 }}>
