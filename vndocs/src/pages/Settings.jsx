...
function Avatar({ name, size = 64, src }) {
  const ini = (name || '').split(' ').filter(Boolean).map(w => w[0]).join('').slice(0, 2).toUpperCase();

  /* Só usa a imagem quando ela é acessível pelo navegador. No ambiente de
     testes o storage devolve "local://...", que não carrega. */
  const usarImagem = src && /^https?:\/\//.test(src);
  ...
  if (usarImagem) {
    return (
      <img
        ...
        style={{ width: size, height: size, borderRadius: '50%', objectFit: 'cover', flexShrink: 0, border: '1px solid rgba(15,23,42,.08)' }}
      />
    );
  }
  return (
    <div style={{ width: size, height: size, borderRadius: '50%', background: 'linear-gradient(135deg,#2563EB,#3B82F6)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#fff', fontWeight: 600, fontSize: size * 0.34, flexShrink: 0 }}>
      {ini || '—'}
    </div>
  );
}
...
function TabPerfil() {
  ...
  /** Abre o seletor e envia a imagem para POST /api/account. */
  ...
  return (
    <div style={{ maxWidth: 560, display: 'flex', flexDirection: 'column', gap: 24 }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 16, padding: 20, background: '#ffffff', border: '1px solid rgba(15,23,42,.07)', borderRadius: 14 }}>
        <Avatar name={name} size={64} src={conta.avatar} />
        <div style={{ flex: 1 }}>
          <div style={{ fontWeight: 600, fontSize: 16, marginBottom: 2 }}>{name}</div>
          <div style={{ fontSize: 13, color: 'rgba(15,23,42,.45)' }}>{email}</div>
        </div>
        <button onClick={trocarFoto} disabled={enviandoFoto} title="Enviar uma imagem de até 2 MB" style={{  height: 30, padding: '0 12px', borderRadius: 999, background: '#ffffff', border: '1px solid rgba(15,23,42,.07)', color: 'rgba(15,23,42,.62)', fontFamily: 'inherit', fontSize: 12.5, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 6 }}>
      ...
      <div style={{ display: 'flex', gap: 10 }}>
        ...
        <button onClick={() => { setName(conta.nome || ''); setEmail(conta.email || ''); setCompany(conta.empresa || ''); setErro(null); }} style={{ height: 38, padding: '0 16px', borderRadius: 999, background: '#ffffff', border: '1px solid rgba(15,23,42,.07)', color: 'rgba(15,23,42,.62)', fontFamily: 'inherit', fontSize: 13.5, cursor: 'pointer' }}>Descartar</button>
      </div>
    </div>
  );
}
...
function TabSeguranca() {
  ...
  const campo = (rotulo) => ({
    'Senha atual': 'atual', 'Nova senha': 'nova', 'Confirmar nova senha': 'confirma',
  }[rotulo]);

  /* Troca a senha de verdade, via POST /api/auth/password. */
          ...
          {['Senha atual', 'Nova senha', 'Confirmar nova senha'].map(l => (
            <div key={l}>
              ...
              <input type="password" placeholder="••••••••"
                value={senhas[campo(l)]}
                onChange={e => setSenhas(s => ({ ...s, [campo(l)]: e.target.value }))}
                onKeyDown={e => { if (e.key === 'Enter') trocarSenha(); }}
                style={{ width: '100%', height: 42, padding: '0 14px', borderRadius: 10, background: '#ffffff', border: '1px solid rgba(15,23,42,.1)', color: '#0f172a', fontSize: 14, outline: 'none' }} />
            </div>
          ))}
          <button onClick={trocarSenha} disabled={trocando} style={{ height: 30, padding: '0 14px', borderRadius: 999, background: trocando ? '#93C5FD' : '#2563EB', color: '#fff', border: 'none', fontFamily: 'inherit', fontSize: 12.5, fontWeight: 600, cursor: 'pointer', alignSelf: 'flex-start' }}>
            {trocando ? 'Trocando…' : 'Atualizar senha'}
          </button>
        </div>
      </div>
      <div style={{ background: '#ffffff', border: '1px solid rgba(15,23,42,.07)', borderRadius: 14, padding: 24 }}>
    ...
    <AppLayout>
      <div style={{ display: 'flex', flexDirection: 'column', height: '100%', overflow: 'hidden' }}>
        ...
        <div style={{ flex: 1, display: 'flex', overflow: 'hidden' }}>
          {/* Sidebar tabs */}
          <div style={{ width: isMobile ? '100%' : 210, borderRight: isMobile ? 'none' : '1px solid rgba(15,23,42,.07)', borderBottom: isMobile ? '1px solid rgba(15,23,42,.07)' : 'none', display: isMobile ? 'flex' : 'block', gap: isMobile ? 8 : 0, overflowX: isMobile ? 'auto' : 'visible', padding: '16px 12px', flexShrink: 0 }}>
