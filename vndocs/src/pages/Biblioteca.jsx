...
import {
  ...
  ChevronRight, Upload, MoreVertical,
} from 'lucide-react';
...
function CartaoArquivo({ tipo, nome, apoio, acoes }) {
  ...
  const gerar = async (alvo) => {
    ...
    try {
      const res = await fetch('/api/generate', {
        method: 'POST', headers: { 'content-type': 'application/json' },
        body: JSON.stringify({ templateId: alvo.templateId, sheetId: alvo.sheets[0].id }),
      });
      if (!res.ok) {
        const corpo = await res.json().catch(() => null);
          ...
          <div style={{
            ...
            {!pasta ? (
              ...
              <>
                ...
                <div style={{ flex: 1 }} />
                <Acao aoClicar={() => gerar(pasta)} desabilitado={!podeGerar}
                  ...
                  <CartaoArquivo
                    ...
                    acoes={[
                      { rotulo: 'Gerar com este modelo', Icone: Zap,
                        aoClicar: () => podeGerar ? gerar(pasta) : toast.warning('Adicione uma planilha primeiro.') },
                  ...
                  <CartaoArquivo
                    ...
                    acoes={[
                      { rotulo: 'Gerar com esta planilha', Icone: Zap,
                        aoClicar: () => pasta.templateId ? gerar(pasta) : toast.warning('Envie um modelo primeiro.') },
