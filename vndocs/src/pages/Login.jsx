...
function TextInput({ label, placeholder, value, onChange, type = 'text', onEnter }) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
      {label && <label style={{ fontSize: 12.5, color: 'var(--n7)', fontWeight: 500 }}>{label}</label>}
      <input
        ...
        style={{
          ...
          background: 'var(--n0)', fontFamily: 'inherit', fontSize: 14, outline: 'none',
          border: `1px solid ${focus ? 'var(--azul)' : 'var(--n4)'}`,
          boxShadow: focus ? '0 0 0 3px var(--azul-veu)' : 'none',
      ...
      <div style={{ position: 'relative' }}>
        ...
        <button
          ...
          style={{
            ...
            width: 30, height: 30, borderRadius: 7, border: '1px solid var(--n3)',
            background: 'var(--n0)', color: 'var(--n6)', cursor: 'pointer',
...
function CartaoPlanilha() {
  ...
  return (
    <div style={{
      background: 'var(--n0)', border: '1px solid var(--n3)', borderRadius: 14,
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 14 }}>
        <div style={{
          ...
          border: '1px solid rgba(22,163,74,.22)', color: 'var(--verde)',
          display: 'flex', alignItems: 'center', justifyContent: 'center',
        }}>
          <Table2 size={15} />
        </div>
        <div>
          ...
          <div style={{ fontSize: 11, color: 'var(--n6)' }}>82 linhas · 7 colunas</div>
        </div>
      </div>
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '9px 10px' }}>
        ...
        {linhas.map(l => l.map((celula, i) => (
          <div key={l[0] + i} style={{ fontSize: 11.5, color: 'var(--n7)' }}>{celula}</div>
        )))}
      </div>
    </div>
  );
...
function CartaoContrato() {
  const campo = (t) => (
    <span style={{
      background: 'var(--azul-veu)', color: 'var(--azul)', borderRadius: 4,
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        color: 'var(--n6)', marginBottom: 12,
      }}>
        CONTRATO PRESTAÇÃO DE SERVIÇOS
      </div>
      <p style={{ fontSize: 12.5, lineHeight: 1.85, color: 'var(--n7)', margin: 0 }}>
        Pelo presente instrumento particular, {campo('{{nome}}')}, portador do
        CPF {campo('{{cpf}}')}, contrata os serviços pelo valor
        de {campo('{{valor}}')} …
      </p>
      <div style={{
        marginTop: 14, paddingTop: 10, borderTop: '1px solid var(--n3)',
        fontFamily: "'Geist Mono', monospace", fontSize: 9.5,
        letterSpacing: '.1em', color: 'var(--n5)',
      }}>
        PÁGINA 1 / 3
      </div>
    </div>
...
function CartaoResultado() {
  return (
    <div style={{
      background: 'linear-gradient(150deg,var(--azul),var(--azul-forte))', borderRadius: 14,
      padding: '15px 18px', width: 210, color: 'var(--n0)',
...
function PainelMarketing() {
  return (
    <div style={{
      flex: 1, minWidth: 0,
      background: 'radial-gradient(760px 560px at 62% 34%, rgba(37,99,235,.11), transparent 64%), var(--n2)',
      display: 'flex', alignItems: 'center', justifyContent: 'center',
      padding: '48px 44px', position: 'relative', overflow: 'hidden',
    }}>
      {/* Malha de fundo: a folha quadriculada de um caderno de cálculo. */}
      <div style={{
        position: 'absolute', inset: 0, pointerEvents: 'none',
        backgroundImage:
          'linear-gradient(rgba(22,24,29,.022) 1px, transparent 1px),' +
          'linear-gradient(90deg, rgba(22,24,29,.022) 1px, transparent 1px)',
      ...
      {/* Coluna central com largura própria: sem isto o conteúdo cola na
          esquerda e sobra um vazio enorme à direita em telas largas. */}
      <div style={{
        ...
        <div>
          <div style={{
            fontFamily: 'var(--fonte-dado)', fontSize: 10.5, color: 'var(--azul)',
          ...
          <h2 style={{
            ...
            fontSize: 'clamp(30px, 3.1vw, 40px)', letterSpacing: '-0.035em',
            lineHeight: 1.1, margin: 0, color: 'var(--n9)',
          }}>
            Um modelo.<br />
            <span style={{ color: 'var(--azul)' }}>Centenas</span> de documentos.
          </h2>

          <p style={{
            fontSize: 'var(--t-corpo)', color: 'var(--n7)', lineHeight: 1.6,
            maxWidth: '46ch', margin: '14px 0 0',
          }}>
            Marque os campos no Word, aponte as colunas da planilha, e receba
            todos os documentos preenchidos num arquivo só.
          </p>
        </div>

        {/* A composição: três peças em escada, do dado ao resultado.
            Empilhadas com margem negativa em vez de posição absoluta —
            assim a coluna acompanha a altura real e nada se sobrepõe. */}
        <div style={{ display: 'flex', flexDirection: 'column' }}>
          <div style={{ alignSelf: 'flex-start', width: 'min(300px, 100%)' }}>
            <CartaoPlanilha />
          </div>
          <div style={{
            alignSelf: 'center', width: 'min(330px, 100%)',
            marginTop: -18, marginLeft: 'clamp(0px, 4vw, 60px)', zIndex: 1,
          }}>
            <CartaoContrato />
          </div>
          <div style={{
            alignSelf: 'flex-end', width: 'min(212px, 100%)',
        ...
        <div style={{
          ...
          paddingTop: 22, borderTop: 'var(--fio)',
        }}>
          {[
            ['2,4 mi', 'Documentos gerados'],
            ['3,1 s', 'Para cem documentos'],
            ['1,8 mil', 'Equipes usando'],
          ].map(([v, l]) => (
            <div key={l}>
              <div style={{
                fontFamily: 'var(--fonte-titulo)', fontWeight: 600, fontSize: 21,
                letterSpacing: '-0.03em', color: 'var(--n9)',
...
export default function Login() {
  ...
  return (
    <div style={{ display: 'flex', minHeight: '100vh', background: 'var(--n1)' }}>
      {/* Formulário */}
      <div style={{
        width: isMobile ? '100%' : 480, flexShrink: 0,
        display: 'flex', flexDirection: 'column', justifyContent: 'center',
        padding: isMobile ? '40px 22px' : '48px 52px', background: 'var(--n1)',
      }}>
        ...
        <p style={{
          fontSize: 14, color: 'var(--n6)', lineHeight: 1.55, margin: 0, marginBottom: 32,
        }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
          ...
          <button
            ...
            style={{
              ...
              background: loading ? '#93C5FD' : 'var(--azul)', color: 'var(--n0)', border: 'none',
        ...
        <p style={{
          fontSize: 11.5, color: 'var(--n6)', lineHeight: 1.6,
          textAlign: 'center', marginTop: 30, marginBottom: 0,
        }}>
          Ao entrar, você concorda com os{' '}
          <span style={{ color: 'var(--n6)' }}>Termos de Uso</span> e a{' '}
          <span style={{ color: 'var(--n6)' }}>Política de Privacidade</span>.
        </p>
      </div>
