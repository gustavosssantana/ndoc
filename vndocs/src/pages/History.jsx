...
export default function History() {
  ...
  const regerar = async (h) => {
    if (!h.podeRegerar) {
      toast.warning('Este lote veio de arquivos enviados na hora, que não ficaram salvos. Gere de novo pela tela Gerar Documentos.');
      return;
    }
    setOcupado(h.id);
    try {
            ...
            <table className="h-ledger">
              ...
              <tbody>
                {loading ? (
                  <tr>
                    <td colSpan={8}>
                      <div className="h-empty"><Spinner size={24} /></div>
                    </td>
                  </tr>
                ) : erroCarga ? (
                  <tr>
                      ...
                      <td style={{ textAlign: 'right' }}>
                        {err ? (
                          <button className="h-act h-act-retry"
                            ...
                            title={h.podeRegerar ? 'Gerar de novo a partir dos arquivos salvos' : 'Os arquivos deste lote não foram salvos na Biblioteca'}>
                          ...
                          <button className="h-act h-act-dl"
                            onClick={() => regerar(h)}
                            disabled={ocupado === h.id || !h.podeRegerar}
                            title={h.podeRegerar ? 'Gerar de novo e baixar o .zip' : 'Os arquivos deste lote não foram salvos na Biblioteca'}>
                            <Download size={12} />
