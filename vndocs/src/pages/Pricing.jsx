        ...
        <div className="stagger" style={{ flex: 1, overflowY: 'auto', padding: '28px 32px', display: 'flex', flexDirection: 'column', gap: 28 }}>
              ...
              return (
                <div key={plan.key} className="hoverable" style={{ background: plan.highlight ? 'linear-gradient(180deg,rgba(37,99,235,.13),rgba(37,99,235,.03) 60%),#ffffff' : '#ffffff', border: isCurrent ? '2px solid #2563EB' : plan.highlight ? '1px solid rgba(37,99,235,.3)' : '1px solid rgba(15,23,42,.07)', borderRadius: 16, padding: '24px 22px', position: 'relative', display: 'flex', flexDirection: 'column', gap: 16 }}>
                  ...
                  <div style={{ marginTop: 'auto' }}>
                    {isCurrent
                      ? <button style={{ width: '100%', height: 38, borderRadius: 999, background: 'rgba(37,99,235,.12)', border: '1px solid rgba(37,99,235,.35)', color: '#2563EB', fontFamily: 'inherit', fontSize: 13.5, fontWeight: 600, cursor: 'default' }}>Plano atual</button>
                      : plan.price === null
                        ? <button style={{ width: '100%', height: 38, borderRadius: 999, background: 'transparent', border: '1px solid rgba(15,23,42,.12)', color: '#0f172a', fontFamily: 'inherit', fontSize: 13.5, cursor: 'pointer' }}>Falar com vendas</button>
                        : plan.price < 79
                          ? <button style={{ width: '100%', height: 38, borderRadius: 999, background: 'transparent', border: '1px solid rgba(15,23,42,.12)', color: '#0f172a', fontFamily: 'inherit', fontSize: 13.5, cursor: 'pointer' }}>Fazer downgrade</button>
                          : <button style={{ width: '100%', height: 38, borderRadius: 999, background: '#2563EB', border: 'none', color: '#fff', fontFamily: 'inherit', fontSize: 13.5, fontWeight: 600, cursor: 'pointer' }} onMouseEnter={e => e.currentTarget.style.background = '#3B82F6'} onMouseLeave={e => e.currentTarget.style.background = '#2563EB'}>Fazer upgrade</button>}
                  </div>
                </div>
              );
            })}
          ...
          <div>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 14 }}>
              ...
              <button style={{ height: 30, padding: '0 12px', borderRadius: 999, background: '#ffffff', border: '1px solid rgba(15,23,42,.07)', color: 'rgba(15,23,42,.7)', fontFamily: 'inherit', fontSize: 12.5, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 6 }}>
                ...
                <tbody>
                  {[['17/05/2025','Plano Pro – Junho 2025','R$ 79,00'],['17/04/2025','Plano Pro – Maio 2025','R$ 79,00'],['17/03/2025','Plano Pro – Abril 2025','R$ 79,00'],['17/02/2025','Plano Pro – Março 2025','R$ 79,00']].map(([date, desc, value]) => (
                    <tr key={date}>
                      ...
                      <td style={{ textAlign: 'right' }}><button style={{ height: 28, padding: '0 10px', borderRadius: 999, background: '#ffffff', border: '1px solid rgba(15,23,42,.07)', color: 'rgba(15,23,42,.6)', fontFamily: 'inherit', fontSize: 12, cursor: 'pointer', display: 'inline-flex', alignItems: 'center', gap: 5 }}><Download size={12} />PDF</button></td>
                    </tr>
                  ))}
                </tbody>
              </table>
          ...
          <div style={{ background: 'linear-gradient(135deg,rgba(37,99,235,.10),rgba(37,99,235,.03)),#ffffff', border: '1px solid rgba(37,99,235,.25)', borderRadius: 14, padding: '24px 28px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            ...
            <button style={{ height: 38, padding: '0 16px', borderRadius: 999, background: '#2563EB', color: '#fff', border: 'none', fontFamily: 'inherit', fontSize: 13.5, fontWeight: 600, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 7 }} onMouseEnter={e => e.currentTarget.style.background = '#3B82F6'} onMouseLeave={e => e.currentTarget.style.background = '#2563EB'}>
