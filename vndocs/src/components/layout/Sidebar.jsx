...
function NavItem({ item, active, navigate, aoNavegar }) {
  const [hov, setHov] = useState(false);
  const { icon: Icon, label, to, badge } = item;
  return (
    <div
      onClick={() => { navigate(to); aoNavegar?.(); }}
...
export default function Sidebar({ aoNavegar }) {
  ...
  return (
    <div style={{
      width: 232, background: '#fff', height: '100%', overflowY: 'auto',
      ...
      <div style={{ flex: 1, padding: '0 12px', display: 'flex', flexDirection: 'column', gap: 2, overflowY: 'auto' }}>
        {NAV_TOP.map(item => (
          <NavItem key={item.to} item={item} active={route === item.to} navigate={navigate} aoNavegar={aoNavegar} />
        ))}
      ...
      <div style={{ padding: 14 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginTop: 12, padding: '10px 4px 2px' }}>
          ...
          <button
            onClick={async () => { aoNavegar?.(); await logout(); navigate('/login', { replace: true }); }}
            title="Sair"
