...
export function AuthProvider({ children }) {
  ...
  /* Qualquer tela que receba 401 avisa por aqui. Recarregar a sessão faz o
     ProtectedRoute encaminhar para o login, em vez de deixar a pessoa numa
     página vazia lendo "Não autenticado". */
  useEffect(() => {
    const aoExpirar = () => { recarregar(); };
    window.addEventListener('ndocs:sessao-expirou', aoExpirar);
    return () => window.removeEventListener('ndocs:sessao-expirou', aoExpirar);
  }, [recarregar]);
