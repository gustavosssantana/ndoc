...
for (const r of protegidas) {
  ...
  if (r === '/biblioteca') {
    ...
    check('  biblioteca mostra os 7 campos', html.includes('7 campos'));
  }
  if (r === '/history') {
...
logado = true;
{
  ...
  if (cartao) {
    ...
    check('celular: tocar na pasta abre o detalhe',
      host.innerHTML.includes('Gerar com esta pasta'));
...
definirLargura(1440);
{
  ...
  check('desktop: lista e detalhe aparecem juntos',
    html.includes('Gerar com esta pasta') && html.includes('Contratos de presta'));
...
check('pasta com modelo oferece trocar', bibliotecaCheia.includes('Trocar'));
...
check('pasta sem modelo oferece enviar',
  bibliotecaVazia.includes('Enviar'), 'não achou o botão');
check('pasta sem modelo explica o que falta',
  bibliotecaVazia.includes('Sem modelo') || bibliotecaVazia.includes('Envie um modelo'),
  'não explicou o que falta');
