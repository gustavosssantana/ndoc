...
/* ── 2. Pasta ─────────────────────────────────────────────────────────── */
const pasta = await repo.criarPasta(user, {
...
const mesmaPasta = await repo.criarPasta(user, { nome: 'Contratos de prestação' });
...
const tpl = await repo.salvarTemplate(user, {
...
const sh = await repo.salvarPlanilha(user, {
...
const pastas = await repo.listarPastas(user);
...
const tplsDepois = await repo.listarTemplates(user);
...
const hist = await repo.listarGeracoes(user);
...
/* ── 8. Apagar ────────────────────────────────────────────────────────── */
const apagou = await repo.apagarPlanilha(user, sh.id);
check('apaga planilha', apagou === true);
const pastasDepois = await repo.listarPastas(user);
check('pasta fica sem planilhas', pastasDepois[0].sheets.length === 0);
const histIntacto = await repo.listarGeracoes(user);
