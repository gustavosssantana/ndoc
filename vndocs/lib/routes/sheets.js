...
export const GET = comConta(async (_req, user) => json(await listarPlanilhas(user)));
...
export const POST = comConta(async (request, user) => {
  ...
  return json(await salvarPlanilha(user, arquivo), 201);
});
...
export const DELETE = comConta(async (request, user) => {
  ...
  const ok = await apagarPlanilha(user, id);
