...
export const GET = comConta(async (_req, user) => json(await listarPastas(user)));
...
export const POST = comConta(async (request, user) => {
  ...
  return json(await criarPasta(user, corpo), 201);
});
...
export const PATCH = comConta(async (request, user) => {
  ...
  const corpo = await request.json().catch(() => ({}));
  try {
    const f = await renomearPasta(user, id, corpo);
...
export const DELETE = comConta(async (request, user) => {
  const id = new URL(request.url).searchParams.get('id');
  if (!id) return json({ error: 'Informe o id da pasta.' }, 400);
  const ok = await apagarPasta(user, id);
