...
export async function POST(request) {
  ...
  if (porId) {
    ...
    const { ler } = await import('../storage.js');
    const tpl = await one(
    ...
    /* Se o armazenamento falhar, a mensagem precisa chegar ao usuário —
       antes o erro subia sem explicação e virava um "Não foi possível gerar". */
    try {
      [templateBuffer, xlsxBuffer] = await Promise.all([
        ler(tpl.blob_pathname), ler(sh.blob_pathname),
      ]);
    } catch (err) {
      console.error('nao foi possivel ler os arquivos salvos:', err);
      return json({
        error: `Não foi possível ler os arquivos salvos: ${err.message}`,
      }, 502);
    }
  } else {
    let upload;
    ...
    templateName = upload.fields.templateName || upload.templateFilename;
    sheetName = upload.fields.sheetName || upload.sheetFilename;
  }
  ...
  try {
    ...
    /* Lote feito por upload: guardamos os arquivos para que o histórico
       consiga refazer o lote depois. Sem isso, o botão Baixar não tem de
       onde tirar nada — o .zip em si não é armazenado. */
    if (!porId) {
      try {
        const [tplSalvo, shSalva] = await Promise.all([
          repo.salvarTemplate(user, {
            filename: templateName, buffer: templateBuffer, folderId: null,
          }),
          repo.salvarPlanilha(user, {
            filename: sheetName, buffer: xlsxBuffer, folderId: null,
          }),
        ]);
        templateId = tplSalvo.id;
        sheetId = shSalva.id;
      } catch (err) {
        /* Não guardar é ruim, mas não justifica negar o lote ao usuário. */
        console.error('lote gerado mas arquivos nao guardados:', err.message);
      }
    }

    {
      try {
        await repo.registrarGeracao(user, {
