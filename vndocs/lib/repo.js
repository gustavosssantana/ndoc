...
export async function listarPastas(userId) {
  ...
  const planilhas = await query(
    `SELECT id, folder_id, filename, row_count, updated_at
       FROM sheets
      WHERE user_id = $1 AND folder_id = ANY($2::uuid[])
...
export async function renomearPasta(userId, id, { nome, descricao }) {
...
export async function apagarPasta(userId, id) {
  const f = await one('DELETE FROM folders WHERE user_id = $1 AND id = $2 RETURNING id', [userId, id]);
  return Boolean(f);
}
