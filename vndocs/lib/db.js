...
export async function semearConta({ email, senha, nome = '', empresa = '' }) {
  ...
  const existente = await one('SELECT id, org_id FROM users WHERE email = $1', [limpo]);
  let orgId = existente?.org_id ?? null;

  /* Conta nova ganha a sua organização: é ela que passa a dona da biblioteca. */
  if (!orgId) {
    const org = await one(
      "INSERT INTO orgs (name, plan) VALUES ($1, 'pro') RETURNING id",
      [empresa || nome || limpo],
    );
    orgId = org.id;
  }
  ...
  return one(
    `INSERT INTO users (email, name, company, plan, password_hash, org_id, org_role)
     ...
     RETURNING *`,
    [limpo, nome, empresa, await gerarHash(senha), orgId],
  );
}
